...
2. For_looker_ready (new): previous full week (Sun-Sat) per-MP aggregate, overwritten each run
...
def read_fb_raw_for_week(service, sheet_id, period_start_str):
    """
    Reads FB_raw rows where period_start matches the given week start date.
    This replaces the rolling-days approach so we always aggregate a clean Sun-Sat week.
    """
    try:
        result = service.spreadsheets().values().get(
    ...
    for row in values[1:]:
        ...
        if row_dict.get("period_start", "") == period_start_str:
            rows.append(row_dict)

    log.info("Read %s rows from FB_raw for week starting %s", len(rows), period_start_str)
    return rows


def build_for_looker(mapping, recent_rows):
    """
    Aggregates FB_raw rows for the current week into per-MP rows.

    Followers: MAX - MIN across the week (new followers approximation).
    Everything else: SUM.
    """
...
def main():
    ...
    # Previous full week: Sunday 00:00 to Saturday 23:59:59 (Perth time, stored as UTC)
    # We use UTC throughout; Perth is UTC+8 so Sunday midnight Perth = Saturday 16:00 UTC.
    # If you want the week boundary to feel like midnight Perth, swap timezone.utc for
    # a pytz/zoneinfo Perth zone. For now UTC is consistent with how the script was built.
    now = datetime.now(timezone.utc)
    days_since_sunday = (now.weekday() + 1) % 7  # Mon=0...Sun=6, so Sun gives 0
    last_sunday = now - timedelta(days=days_since_sunday)
    week_start = last_sunday.replace(hour=0, minute=0, second=0, microsecond=0)
    week_end = week_start + timedelta(days=6, hours=23, minutes=59, seconds=59)

    since_ts = int(week_start.timestamp())
    until_ts = int(week_end.timestamp())

    run_date = now.strftime("%Y-%m-%d")
    period_start = week_start.strftime("%Y-%m-%d")
    period_end = week_end.strftime("%Y-%m-%d")

    log.info("Run: %s | week %s to %s", run_date, period_start, period_end)
    ...
    # Step 2: pull metrics for the previous week
    data_rows = []
    ...
    # Step 5: build For_looker_ready from this week's FB_raw rows
    mapping = read_mapping(service, sheet_id)
    if mapping:
        recent = read_fb_raw_for_week(service, sheet_id, period_start)
